// aboutEchart/pages/zheXian/index.js
import * as echarts from '../../comps/ec-canvas/echarts';
import moment from 'moment'
import { diseaseOption } from './options'
...
Page({
  ...
  init_diseaseChart(data) {
    this.diseaseChartComponnet = this.selectComponent('#diseaseChart');        //去获取echarts    这里的id就是echarts的id
    this.diseaseChartComponnet.init((canvas, width, height, dpr) => {
      ...
      diseaseChart.setOption(diseaseOption({
        chartData: moment(data.firstReport.checkDate * 1000).format('YYYY.MM.DD') == moment(data.lastReport.checkDate * 1000).format('YYYY.MM.DD') ?
          [data.lastReport] :
          [data.firstReport, data.lastReport]
